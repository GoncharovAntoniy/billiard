...
const BascketCard = ({ ...card }) => {
  ...
  function increment(id) {
    ...
    axios.patch(
      `http://127.0.0.1:8000/api/cue/${id}/shopping_cart/`,
      {
        count: card.count + 1,
      },
      {
  ...
  const cartDelete = (id) => {
    ...
    axios.delete(`http://127.0.0.1:8000/api/cue/${id}/shopping_cart/`, {
      headers: {
        "content-type": "application/json",
        authorization: `Token ${token}`,
      },
  ...
  return (
    <div>
      <div className={s.card}>
        ...
        <button id={card.cue.id} className={s.card_bascket_button}>
          Оформить заказ
        </button>
        <button
